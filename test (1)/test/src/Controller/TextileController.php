...
final class TextileController extends AbstractController
{
    ...
    #[Route('/back', name: 'app_textile_back', methods: ['GET'])]
    public function back(Request $request, TextileRepository $textileRepository, CollectionTRepository $collectionRepository): Response
    {
        $collections = $collectionRepository->findAll();
        $selectedCollectionId = $request->query->get('collection_id');
        $user=$this->getUser();
        ...
        return $this->render('textile/BackTextile.html.twig', [
            'textiles' => $textiles,
            'collections' => $collections,
            'selectedCollection' => $selectedCollectionId ? $collectionRepository->find($selectedCollectionId) : null,
            'user'=>$user
    ...
    #[Route('/{id}/edit/back', name: 'app_textile_edit_back', methods: ['GET', 'POST'])]
    public function editb(Request $request, Textile $textile, EntityManagerInterface $entityManager, SluggerInterface $slugger, ParameterBagInterface $params): Response
    {
        $form = $this->createForm(TextileType::class, $textile);
        $form->handleRequest($request);
        $user=$this->getUser();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('app_textile_back', [], Response::HTTP_SEE_OTHER);
        }
    
        return $this->render('textile/editback.html.twig', [
            'textile' => $textile,
            'form' => $form->createView(),
            'user'=>$user
        ]);
