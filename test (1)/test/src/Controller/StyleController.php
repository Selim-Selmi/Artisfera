...
final class StyleController extends AbstractController
{
  ...
  /*   #[Route('/new', name: 'app_style_new')]
    ...
    #[Route('/list/back/style', name: 'app_style_list_back')]
    public function fffffffffff(StyleRepository $styleRepository){
        $peintureBD = $styleRepository->findAll();
        $us=$this->getUser();
        return $this->render('style/BackStyle.html.twig',[
            'styles' => $peintureBD ,
            'user'=>$us
        ]);
    }

    #[Route('/style/back/edit/{id}', name: 'app_style_edit_back')]
    public function editStylee($id, Request $request, EntityManagerInterface $em, StyleRepository $styleRepository): Response
    {
        $style = $styleRepository->find($id);
        $form = $this->createForm(AddEditStyleType::class, $style);
        $form->handleRequest($request);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('app_style_list_back');
        }

        return $this->render('style/editStyleBack.html.twig', [
            'title' => 'Update Style',
            'form' => $form->createView(),
            'style' => $style,
            'user'=>$us,
    ...
    #[Route('/style/back/remove/{id}', name: 'app_style_remove_back')]
    public function removeStylee($id, StyleRepository $styleRepository, EntityManagerInterface $em){
        $style= $styleRepository->find($id);
        $em->remove($style);
        $em->flush();
        return $this->redirectToRoute('app_style_list_back');
    ...
    #[Route('/new/style', name: 'app_style_new_back')]
    public function newStylee(Request $request, EntityManagerInterface $em): Response
    {
        $style = new Style();
        $user=$this->getUser();
        $style->setUserId($user->getId());
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($imageFile) {
                ...
                try {
                    ...
                    $this->addFlash('error', 'Erreur lors de l\'upload de l\'image.');
                    return $this->redirectToRoute('app_style_new_back');
                }
